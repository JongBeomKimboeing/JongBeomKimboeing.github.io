...
### 2) 포아송 분포의 필요성
-> 연속된 시간 상에서 발생하는 사건은 매 순간 발생 가능하다.
시행 횟수가 많고 순간의 성공 확률은 작기 때문에 이항분포로 설명하기 어렵다.
그러므로, 포아송 분포를 이용하여 단위시간/공간에 드물게 나타나는 사건의 횟수를 나타낸다.

### 3) 포아송 분포의 예시
1) 일정 시간동안 발생하는 불량풍의 수
2) 일정 시간동안 톨게이트를 지나는 차량의 수
3) 일정 페이지의 문장을 완성했을 때 발생하는 오타의 수
